...
/**
 * This object responsible for sending the request to the correct device and receiving its answer. In case of the PC Server does
 * not responds at first probe, the object is trying to refresh the target's IP from the Main Server once.
 * 
...
public class PCFileProvider implements PCFileService {
	...
	/**
	 * Local Android Database Service, with help of this we can get the stored password of the current server from the local
	 * database.
	 */
	private AndroidDatabaseService dbService = null;

	/**
	 * If the PC Server does not responds the system trying to get the correct PC Server's IP address from the Main Server. If
	 * this variable not equals with 0 the system do not try to get the correct IP address because it means that it tried already.
	 */
	...
	/**
	 * @see #getListOfFiles(String, String)
	 */
	@Override
		...
		try {
			...
			if (errCode != 1) { // If the response were not correct or it were an error message.
				if (errCode == AErrorMessageInterpreter
						...
						&& this.NUMBER_OF_PROBES++ == 0) { // PC Server is offline or it have a new IP.
	...
	/**
	 * @see #getDirectorySize(String, String)
	 */
	@Override
	...
	/**
	 * Send the request message to the PC Server or the Main Server.
	 * 
	 * @param URL
	 *            Target URL.
	 * @param xmlMessage
	 *            XML formated message.
	...
	/**
	 * Updates the given Server IP and Port.
	 * 
	 * @param serverName
	 *            Target Server name.
	 * @param ip
	 ...
	 * @param port
	 *            New Port.
	 * @return True - in case of successfully update / False - otherwise.s
	 */
